...
// .wrangler/tmp/bundle-eQfE7d/checked-fetch.js
...
// .wrangler/tmp/bundle-eQfE7d/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-eQfE7d/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
var getAllowedOrigin = /* @__PURE__ */ __name((requestOrigin) => {
...
function json(d2, req, extraHeaders = {}) {
...
__name(json, "json");
function error(m2, s2, req) {
  const origin = req?.headers.get("Origin");
  return new Response(JSON.stringify({ error: m2 }), {
    status: s2,
    headers: {
      "Content-Type": "application/json",
      ...corsHeaders(getAllowedOrigin(origin))
...
async function generatePresignedUrl(env, key, contentType, contentLength, cacheControl) {
...
async function getFileFromS3(env, key) {
  const S3 = getS3Client(env);
  try {
...
__name(getFileFromS3, "getFileFromS3");

// worker/src/handlers.js
async function createSession(req, env, config) {
...
async function createFile(req, env, config) {
  ...
  const cacheControl = `public, max-age=${Math.floor(config.EXPIRY_MS / 1e3)}`;
  let uploadUrl;
  try {
...
function getConfig(env) {
  return {
    ...
    // Default to empty, allowing fallback
    // Limits & Quotas
    MAX_FILE_SIZE: parseInt(env.MAX_FILE_SIZE) || 100 * 1024 * 1024,
    // 100MB
    RATE_LIMIT_HITS: parseInt(env.RATE_LIMIT_HITS) || 20,
    EXPIRY_MS: parseInt(env.EXPIRY_MS) || 60 * 60 * 1e3,
    // 60 minutes
    // Storage
          ...
          return await withAuth(req, env, ctx, config, async () => {
            ...
            return new Response(JSON.stringify({
              ...
              headers: {
                ...
                // Fallback * if no origin? No, credentials:true forbids *.
                "Access-Control-Allow-Credentials": "true"
              }
            });
          });
